...
describe('custom station import', () => {
  it('imports a URL from command mode and shows newest imports between favorites and recents', async () => {
    const {store, service} = fixture();
    store.addImported([{id: 'older-import', provider: 'playlist', name: 'Older import', tags: ['custom'], streamUrl: 'https://example.test/older'}]);
    store.addRecent({id: 'recent', provider: 'radio-browser', name: 'Recently played', tags: [], streamUrl: 'https://example.test/recent'});
    vi.stubGlobal('fetch', async () => new Response('', {
      headers: {'content-type': 'audio/aac', 'icy-name': 'Imported from headers', 'icy-br': '64'}
    }));

    const app = render(<App store={store} alarmService={service} />);
    await settle();
    app.stdin.write(':import https://streams.example.test/live');
    await settle();
    app.stdin.write('\r');
    await settle();

    expect(store.snapshot().imported.map(item => item.name)).toEqual(['Imported from headers', 'Older import']);
    const frame = app.lastFrame() ?? '';
    expect(frame).toContain('Imported Imported from headers · AAC · 64 kbps.');
    expect(frame.indexOf('KEXP')).toBeLessThan(frame.indexOf('Imported from headers'));
    expect(frame.indexOf('Imported from headers')).toBeLessThan(frame.indexOf('Recently played'));
    app.unmount();
  });
});
