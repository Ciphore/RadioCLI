...
describe('alarm runner',()=>{
  ...
  it('stops playback again when a canceled readiness operation succeeds late',async()=>{let finish:()=>void=()=>{};let active=false;const stop=vi.fn(async()=>{active=false;});const play=vi.fn(()=>new Promise<void>(resolve=>{finish=()=>{active=true;resolve();};}));const d=deps({player:{play,stop,setVolume:vi.fn(async()=>({ok:true})),getState:vi.fn(()=>({backend:'mpv'}))},subscribeSignals:handler=>{setTimeout(handler,0);return vi.fn();}});const result=await runAlarm('a',scheduledAt,d);expect(result.status).toBe('dismissed');expect(stop).toHaveBeenCalledTimes(2);finish();await Promise.resolve();await Promise.resolve();expect(stop).toHaveBeenCalledTimes(3);expect(active).toBe(false);});
  ...
  it('releases an inhibitor that finishes acquiring after cancellation',async()=>{let finish:(lease:InhibitorLease)=>void=()=>{};const release=vi.fn(async()=>{});const acquire=vi.fn(()=>new Promise<InhibitorLease>(resolve=>{finish=resolve;}));const d=deps({inhibitor:{status:vi.fn(()=>({supported:true,active:false,message:'ok'})),acquire},subscribeSignals:handler=>{setTimeout(handler,0);return vi.fn();}});const result=await runAlarm('a',scheduledAt,d);expect(result.status).toBe('dismissed');finish({release});await Promise.resolve();await Promise.resolve();expect(release).toHaveBeenCalled();});
