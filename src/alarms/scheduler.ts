...
export class SchedulerService {
  constructor(readonly adapter: SchedulerAdapter, private readonly now: () => Date = () => new Date(),readonly health=new AlarmRuntimeHealthStore(),private readonly guard?:{start(alarm:Alarm):Promise<unknown>;stop(alarmId?:string):Promise<boolean>;status?():Promise<{guards:Array<{alarmId:string}>}>|{guards:Array<{alarmId:string}>}}) {}
  ...
  async runtimeStatus(alarms:readonly Alarm[]=[]){return{capabilities:this.doctor(),entries:this.health.list(),alarms:await this.statusAll(alarms)};}
...
export function createSchedulerAdapter(deps: SchedulerDeps = {}): SchedulerAdapter {
  ...
  if (platform === 'darwin') return launchdAdapter(common);
  if (platform === 'win32') return windowsAdapter(common);
  if (platform === 'linux' && commandExists('systemctl')) return systemdAdapter(common);
...
function runCommand(command:string,args:string[]):Promise<CommandResult>{return new Promise(resolve=>{const child=spawn(command,args,{stdio:['ignore','pipe','pipe'],windowsHide:true});let stdout='';let stderr='';child.stdout.on('data',v=>stdout+=String(v));child.stderr.on('data',v=>stderr+=String(v));child.on('error',e=>resolve({code:127,stdout,stderr:e.message}));child.on('close',code=>resolve({code:code??1,stdout,stderr}));});}
