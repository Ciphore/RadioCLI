...
describe('public network policy', () => {
  ...
  it('accepts requests explicitly bypassed by NO_PROXY before validating proxy configuration', async () => {
    vi.stubEnv('HTTPS_PROXY', 'socks5://unsupported.invalid:1080');
    vi.stubEnv('NO_PROXY', '.example.com,localhost:8443');
    const fetchImpl = vi.fn(async () => new Response('{"ok":true}'));

    await expect(withExternalResponse('https://api.example.com/data', {timeoutMs: 100, fetchImpl}, response => response.json())).resolves.toEqual({ok: true});
    await expect(withExternalResponse('https://localhost:8443/data', {timeoutMs: 100, fetchImpl}, response => response.json())).resolves.toEqual({ok: true});
    await expect(withExternalResponse('https://localhost:9443/data', {timeoutMs: 100, fetchImpl}, response => response.json())).rejects.toThrow(/SOCKS/i);
    expect(fetchImpl).toHaveBeenCalledTimes(2);
    expect(allowsDirectDnsFallback(process.env, 'https://api.example.com')).toBe(true);
  });
