...
export function allowsDirectDnsFallback(env: NodeJS.ProcessEnv = process.env, url?: string | URL): boolean {
  if (url && bypassesProxy(new URL(url), env)) return true;
...
function validateProxyConfiguration(url: URL): void {
  if (bypassesProxy(url, process.env)) return;
...
function bypassesProxy(url: URL, env: NodeJS.ProcessEnv): boolean {
