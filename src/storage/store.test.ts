...
describe('JsonLibraryStore', () => {
  ...
  it('places newly imported stations before older imports', () => {
    const root = mkdtempSync(join(tmpdir(), 'radiocli-import-order-'));
    roots.push(root);
    const store = new JsonLibraryStore(join(root, 'library.json'));
    const custom = (id: string): Station => ({
      id,
      provider: 'playlist',
      name: id,
      tags: ['custom'],
    ...
    store.addImported([custom('older')]);
    store.addImported([custom('newest'), custom('second-newest')]);
    expect(store.snapshot().imported.map(item => item.id)).toEqual(['newest', 'second-newest', 'older']);

    store.addImported([custom('older')]);
    expect(store.snapshot().imported.map(item => item.id)).toEqual(['older', 'newest', 'second-newest']);
  });
