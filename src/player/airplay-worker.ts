...
import {parseWorkerMessage, decodeWorkerStart, serializeWorkerMessage, type AirPlayWorkerCommand, type AirPlayWorkerEvent} from './airplay-worker-protocol.js';
...
const start = decodeWorkerStart(encodedStart);
...
function loadAirTunesSender(): new () => AirTunesClient {
  try {
    ...
    emit({type: 'error', message: 'AirPlay sender package not installed. Install optional peer node-airtunes2 next to RadioCLI.'});
    process.exit(1);
  }
}
