...
- Experimental macOS output: AirPlay/RAOP through Bonjour discovery, `ffmpeg`,
  and a sender package that passes RadioCLI's dependency safety gate
...
compatible sender package. The backend is only advertised when the sender passes
the dependency safety gate; current vulnerable `node-airtunes2` installs stay
blocked instead of being loaded opportunistically.
