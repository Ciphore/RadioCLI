...
## Custom Station And Playlist Import
...
Use `radiocli import` with a direct HTTP(S) stream when a station is missing from
search or from the open provider directories. This is especially useful for
networks such as iHeartMedia whose stations may not be available to open-source
radio directories. Look up the station's direct stream URL—not merely its website
or player page—and import it:

```bash
radiocli import https://example.com/live.mp3
```

RadioCLI makes a short, bounded request for HTTP and ICY headers and uses any
published station name, homepage, codec, bitrate, and HLS status. Supply a name
only when the stream does not publish one or when you want to override it:

```bash
radiocli import https://example.com/live.mp3 "Example FM"
```

The submitted stable URL is retained instead of temporary signed redirects.
Imports are stored newest first and displayed after favorites but before recently
played stations. Metadata availability varies by broadcaster, so details that a
stream does not publish cannot always be discovered automatically.
