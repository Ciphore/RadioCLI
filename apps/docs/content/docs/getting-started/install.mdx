...
- `mpv` for playback, pause, mute, volume, media keys, metadata, and readiness
  checks
- `ffmpeg` plus an audited, compatible sender package on macOS for experimental
  AirPlay
...
On macOS with npm, install `mpv` first. Install FFmpeg too if you want to test
the experimental AirPlay backend:
...
`ffmpeg`, but it only advertises the `airplay` backend when a compatible sender
package passes its dependency safety gate. Do not install `node-airtunes2`
blindly: the current public package line is blocked by RadioCLI because its
transitive dependency tree contains known vulnerable versions. If you supply a
patched compatible sender, passcode-protected receivers prompt in the TUI; enter
the code with `:airplay-code 1234`.
