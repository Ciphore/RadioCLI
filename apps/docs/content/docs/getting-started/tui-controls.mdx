...
| `space` / `F8` | Pause; resume reconnects at the live broadcast |
| `t` | Cycle display color |
| `v` | Cycle receiver style |
| `+` / `-` | Adjust volume |
| `f` | Favorite the selected or playing station outside active text input |
| `Ctrl+F` | Favorite the selected Search result while its query input is active |
...
Because RadioCLI plays live radio rather than on-demand audio, resuming a paused
station reloads its stream and discards buffered delayed audio. This returns to
what the station is broadcasting now for imported and directory stations alike.
...
| Search input | type query, `Up` / `Down` move results, `Ctrl+Up` / `Ctrl+Down` recall search history, `Enter` search or tune, `Ctrl+F` favorite, `Esc` finish |
...
- a next-occurrence preview, a no-state-change test tune, Save, and Cancel
...
:stats
:settings
:import https://example.com/live.mp3
:import https://example.com/live.mp3 Example FM
:export radiocli-backup.json
:restore radiocli-backup.json
:stop
```

`:import` adds a custom station from a direct HTTP(S) stream URL and discovers
available HTTP/ICY metadata automatically. Use it when a station is absent from
search or an open-source radio directory, including direct streams from networks
such as iHeartMedia. Find the station's direct stream URL rather than its ordinary
website or web-player page. Newly imported stations appear directly below
favorites and above recently played stations in the Library.

`:restore` is reserved for restoring a full RadioCLI JSON backup containing
preferences and library data.
