...
packages from lifecycle scripts. Native playback comes from `mpv`, with `ffplay`
from FFmpeg as an optional fallback. Linux users install `mpv` through their
distro package manager. Windows users install Node.js and `mpv` through `winget`
or Scoop, then install RadioCLI through npm.
