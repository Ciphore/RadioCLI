...
AirPlay is experimental on macOS and is not shipped with a bundled sender
dependency. It should only appear when `ffmpeg`, Bonjour `dns-sd`, and a
compatible sender package pass RadioCLI's dependency safety gate; current
`node-airtunes2` installs are blocked when vulnerable transitive dependencies
are present.
