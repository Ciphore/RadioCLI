...
![RadioCLI full-screen Now Playing receiver](/demo/radiocli-fullscreen.png)
...
- a braille Explore world map beside the station list, with click-to-place mouse
  ...
  country lists, global search, a station-density country map, and opt-in nearby
  discovery
- `mpv` playback first, `ffplay` fallback when available
- a receiver-style Now Playing screen with 50 selectable displays, stream
  diagnostics, cleaned ICY metadata, volume, pause, mute, sleep timer, favorite
  state, station skipping, and zero-signal graphics whenever playback is inactive
