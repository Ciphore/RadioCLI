...
- a braille Explore world map beside the station list, backed by a cached
  geotagged station atlas with true distance ranking, plus country lists, global
  search, a station-density country map, and opt-in nearby discovery
...
- a receiver-style Now Playing screen with stream diagnostics, cleaned ICY metadata,
  volume, pause, mute, sleep timer, favorite state, station skipping, and zero-signal
  graphics whenever playback is inactive
