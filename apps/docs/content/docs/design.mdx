...
waves, raindrops, spinning donut, and starfield displays. The extended style
cycle also includes the Termflix-inspired catalog as deterministic, pure-frame
terminal animations so they fit Ink rendering and still freeze to a truthful
zero-signal frame when playback is inactive.
